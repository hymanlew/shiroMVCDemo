 ...
 * <p>
 * springDataRedis RedisTemplate 的使用参照 springbootdemo 项目。
 */
public final class RedisUtil {
    ...
    private static JedisPool getPool() {
        if (pool == null) {
            ...
            config.setMaxTotal(Integer.valueOf(Constant.getProperty("jedis.MaxTotal", "").trim()));
            //控制一个pool最多有多少个状态为idle(空闲的)的jedis实例。
            config.setMaxIdle(Integer.valueOf(Constant.getProperty("jedis.MaxIdle", "").trim()));
            //表示当borrow(引入)一个jedis实例时，最大的等待时间，如果超过等待时间，则直接抛出JedisConnectionException；
            config.setMaxWaitMillis(Integer.valueOf(Constant.getProperty("jedis.MaxWaitMillis", "").trim()));
            //在borrow一个jedis实例时，是否提前进行validate操作；如果为true，则得到的jedis实例均是可用的；
            config.setTestOnBorrow(true);


            String redisIp = Constant.getProperty("jedis.host", "").trim();
            Integer redisPort = Integer.valueOf(Constant.getProperty("jedis.port", "").trim());
            String redisPass = Constant.getProperty("jedis.password", "").trim();
            Integer timeOut = Integer.valueOf(Constant.getProperty("jedis.poolTimeout", "").trim());

            pool = new JedisPool(config, redisIp, redisPort, timeOut, redisPass);
        }
        return pool;
    }
